document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("studentForm").addEventListener("submit", async (e) => {
    ...
    if (studentId) {
      ...
      const response = await fetch(`https://localhost:7295/api/Students/${studentId}`, {
        method: "PUT",
        body: formData
      });
    ...
    formData.append("Gender", gender);
      ...
      const response = await fetch(`https://localhost:7295/api/Students`, {
        method: "POST",
        body: formData
      });
...
async function fetchStudents(batchId) {
  const response = await fetch(`https://localhost:7295/api/Batches/Students/${batchId}`);
  const result = await response.json();
...
async function removeStudent(id) {
  ...
  if (confirm.isConfirmed) {
    const response = await fetch(`https://localhost:7295/api/Students/${id}`, {
      method: "DELETE"
    });
