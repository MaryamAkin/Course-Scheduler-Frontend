...
async function loadInstructors() {
    try {
        const response = await fetch(`https://localhost:7295/api/Topic/instructors-assigned/${topicId}`);
...
async function removeInstructor(instructorId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch(`/api/Topic/${topicId}/RemoveInstructor/${instructorId}`, {
                    method: "DELETE",
                });
...
async function loadAvailableInstructors() {
    try {
        const response = await fetch(`https://localhost:7295/api/Instructors/instructors`);
