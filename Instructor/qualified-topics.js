...
async function loadQualifiedTopics() {
    try {
        const res = await fetch(`https://localhost:7295/api/Topic/assigned-toinstructors/${instructorId}`);
...
async function openAssignModal() {
    try {
        // Fetch all topics
        const topicsRes = await fetch(`https://localhost:7295/api/Topic/all-topics`);
        const topicsData = await topicsRes.json();
        const topics = topicsData.data || [];
        // Fetch instructor's current qualified topics
        const qualifiedRes = await fetch(`https://localhost:7295/api/Topic/assigned-toinstructors/${instructorId}`);
        ...
        if (selected && selected.length > 0) {
            await fetch(`https://localhost:7295/api/Topic/assign-topic`, {
                method: "POST",
