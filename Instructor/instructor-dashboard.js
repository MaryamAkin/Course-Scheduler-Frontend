...
async function loadInstructorInfo() {
    ...
    const res = await fetch(`https://localhost:7295/api/Instructors/profile-currentuser`, {
...
async function loadStats() {
    const token = localStorage.getItem("authToken");
    const res = await fetch(`https://localhost:7295/api/Instructors/instructor-stats`, {
        headers: { "Authorization": `Bearer ${token}` }
    });
...
async function loadUpcomingClasses() {
    const token = localStorage.getItem("authToken");
    const res = await fetch(`https://localhost:7295/api/Periods/instructor-upcoming`, {
        headers: { "Authorization": `Bearer ${token}` }
    });
